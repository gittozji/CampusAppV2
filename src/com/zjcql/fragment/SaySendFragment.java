...
* 吐槽发布
* @author phlofy
* @date 2016年1月2日 下午8:06:24 
*/
public class SaySendFragment extends BaseFragment implements OnClickListener,GeneralListener{
		/**
		 * 根View
		 */
	View mRootView;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 初始化图片选择器
		mImageSelector = new ImageSelector(getActivity(), 9);
		mImageSelector.setGeneralListener(this);
	}
	
	@Override
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		mRootView = inflater.inflate(R.layout.fragment_say_send, null);
		findView();
	...
	private void sendATask() {
		if(mContent.getText().toString().length() < 1){
			toast("内容还未填写");
			return;
		}
		...
		UploadTask task = new UploadTask(bundle) {
			...
			private void sendForText(List<BmobFile> arg0, List<String> arg1){
				Says says = new Says();
				says.setFromUser(user);
				says.setContent(getBundle().getString("content"));
				if(arg0 != null){
					says.setFiles(arg1.toArray(new String[arg1.size()]));
					String[] fileName = new String[arg0.size()];
					int i=0;
					for(BmobFile file : arg0){
						fileName[i++] = file.getUrl();
					}
					says.setFilesName(fileName);
				}
				says.save(MyApplication.getInstance(), new SaveListener() {
